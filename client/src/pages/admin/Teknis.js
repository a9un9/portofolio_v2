...
function Teknis() {
  ...
  const fetchData = () => {
    ...
    API.get('/teknis')
      .then(res => {
        setTeknis(Array.isArray(res.data) ? res.data : []);
        setLoading(false);
      })
      .catch(err => {
        console.error(err);
  ...
  const openModal = (data = null) => {
    if (data) {
      setFormData({
        ...
        level_keahlian: data.level_keahlian ?? 'Pemula',
      });
    } else {
      setFormData({
        id: '',
  ...
  const handleSubmit = (e) => {
    ...
    const url = formData.id ? `/teknis/${formData.id}` : '/teknis';
  ...
  const confirmDeleteHandler = () => {
    API.delete(`/teknis/${idToDelete}`)
      .then(() => {
        setTeknis(teknis.filter(item => item.id !== idToDelete));
  ...
  const filteredData = teknis.filter(item =>
    ...
    <div style={{ display: 'flex' }}>
      ...
      <div style={{ marginLeft: sidebarOpen ? '200px' : '50px', width: '100%', transition: 'margin-left 0.3s' }}>
        ...
        <main style={{ padding: '2rem', minHeight: '80vh', marginTop: '60px' }}>
          <h3 className="mb-4">Keahlian Teknis</h3>
        ...
        <Modal show={modalOpen} onHide={() => setModalOpen(false)}>
          <Modal.Header closeButton>
            <Modal.Title>{formData.id ? 'Edit Keahlian' : 'Tambah Keahlian'}</Modal.Title>
          </Modal.Header>
